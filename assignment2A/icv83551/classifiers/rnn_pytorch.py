...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # (2)
        words_vectors = word_embedding_forward(captions_in, W_embed)
        
        #(3)
        if self.cell_type == "rnn":
            h = rnn_forward(words_vectors, h0, Wx, Wh, b)
        elif self.cell_type == "lstm":
            h = lstm_forward(words_vectors, h0, Wx, Wh, b)
    ...
    def sample(self, features, max_length=30):
        ...
        current_word_idx = torch.full((N,), self._start, dtype=torch.long, device=features.device)
        
        for t in range(max_length):
            # (1)
            word_embed = word_embedding_forward(current_word_idx, W_embed)              
            ...
            # (4)
            captions[:,t] = torch.argmax(scores, dim=1)
            
            # For next loop iteration
            current_word_idx = captions[:,t]
            
            # Chek if the word is <END>
            if (current_word_idx == self._end).all():
                break
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
