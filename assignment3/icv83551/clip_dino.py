...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    with torch.no_grad():
        images_features = clip_model.encode_image(images_tensor)
        
    text_tokens = clip.tokenize(class_texts).to(device)
    with torch.no_grad():
        text_features = clip_model.encode_text(text_tokens)
    
    similarity = get_similarity_no_loop(text_features, images_features)
    
    best_class_indices = torch.argmax(similarity, dim=0).cpu().tolist()
    pred_classes = [class_texts[i] for i in best_class_indices]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        images_tensor = torch.cat(processed_images, dim=0).to(device)
        images_features = clip_model.encode_image(images_tensor)
        self.image_features_norm = F.normalize(images_features, p=2, dim=-1)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        pass
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        similarity = text_features_norm @ self.image_features_norm.T
        _,top_indices = torch.topk(similarity[0], k, largest=True)
        ...
        for i in range(num_iters):
            ...
            if (i + 1) % 100 == 0:
                with torch.no_grad():
                    ...
                    acc = num_correct / Y_train.size(0)
                    print(f'Iter {i+1}/{num_iters}, Loss: {loss.item():.4f}, Acc: {acc:.2%}')
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        pass
