...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        x_start = x_t * sqrt_recip_alphas_cumprod - noise * sqrt_recipm1_alphas_cumprod
        ####################################################################
        return x_start
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        ####################################################################
        # TODO:
        # Transform x_t and x_start to get noise according to Eq.(4) and Eq.(14).
        # Look at the coeffs in `__init__` method and use the `extract` function.
        ####################################################################
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        # 2.
        if self.objective == "pred_noise": # if predict noise, find x_start
            pred_x_start = self.predict_start_from_noise(x_t, t, model_output)
        else: # if predict x_start, find noise
    ...
    def q_sample(self, x_start, t, noise):
        ...
        mu = sqrt_alphas_cumprod_t*x_start
        x_t = sigma * noise + mu
        ####################################################################
        return x_t
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        predictions = self.model(x_t, t, model_kwargs)
        losses = loss_weight * F.mse_loss(predictions, target, reduction="none")
        loss = losses.mean()
        ####################################################################
