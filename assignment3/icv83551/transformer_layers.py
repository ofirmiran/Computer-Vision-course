...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        j = torch.arange(0, embed_dim, 2)
        
        pe[0, :, 0::2] = torch.sin(i * 10000**(-j / embed_dim))
        pe[0, :, 1::2] = torch.cos(i * 10000**(-j / embed_dim))
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # 1) (N, S, E) -> (N, S, H, D_h) -> (N, H, S, D_h)
        q = q.view(N, S, H, D_h).transpose(1, 2)
        k = k.view(N, T, H, D_h).transpose(1, 2)
        v = v.view(N, T, H, D_h).transpose(1, 2)
        # 2) Multiply query and key
        attention_weights_normalize = torch.matmul(q, k.transpose(-2, -1)) / (D_h ** 0.5) # (N, H, S, D_h) @ (N, H, D_h, T) -> (N, H, S, T)

        # 3)
        if attn_mask is not None:
            attention_weights_normalize = attention_weights_normalize.masked_fill(attn_mask == 0, float('-inf'))
        
        # Softmax and dropout
        probabilities = torch.softmax(attention_weights_normalize, dim=-1)
        probabilities_drop = self.attn_drop(probabilities)
        
        # Multiply value
        output = torch.matmul(probabilities_drop,v) # (N, H, S, T) @ (N, H, T, D_h) -> (N, H, S, D_h)
        output = output.transpose(1, 2).contiguous().view(N, S, E) # (N, H, S, D_h) -> (N, S, H, D_h) -> (N, S, E)
        
        # Multiply the projection layer
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = x.reshape((N,C,patch_H,self.patch_size,patch_H,self.patch_size))
