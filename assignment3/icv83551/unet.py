...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        model_kwargs_conditional = copy.deepcopy(model_kwargs)
        model_kwargs_unconditional = copy.deepcopy(model_kwargs)
        model_kwargs_unconditional["text_emb"] = None
        
        eps_conditional = self. forward (x, time, model_kwargs_conditional)
        eps_unconditional = self. forward (x, time, model_kwargs_unconditional)

        x = (cfg_scale + 1) * eps_conditional - cfg_scale * eps_unconditional
        ##################################################################
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        for upsample, block1, block2 in self.ups:
            x = upsample(x)
            x = torch.cat((x, feature_list.pop()), dim=1)
            x = block1(x, context)
            x = torch.cat((x, feature_list.pop()), dim=1)
            x = block2(x, context)
        ##################################################################
