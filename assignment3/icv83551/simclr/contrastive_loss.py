...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        denominator_right = 0
        for i in range(2*N):
            if i != k: denominator_left += torch.exp(sim(z_k, out[i])/tau)
            if i != k+N: denominator_right += torch.exp(sim(z_k_N, out[i])/tau)
        
        total_loss += -torch.log(numerator/denominator_left) - torch.log(numerator/denominator_right)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # HINT: torch.linalg.norm might be helpful.                                  #
    ##############################################################################
    multiplication = torch.mul(out_left, out_right).sum(dim=1, keepdim=True)
    norm_left = torch.linalg.norm(out_left, dim=1, keepdim=True)
    norm_right = torch.linalg.norm(out_right, dim=1, keepdim=True)
    
    pos_pairs = multiplication/torch.mul(norm_left, norm_right)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return pos_pairs
...
def compute_sim_matrix(out):
    ...
    multiplication = torch.matmul(out, out.T)
    norm = torch.linalg.norm(out, dim=1, keepdim=True) #[2N,1]
    norm_matrix = torch.matmul(norm, norm.T)

    sim_matrix = multiplication/norm_matrix
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # Step 1: Use sim_matrix to compute the denominator value for all augmented  #
    # samples.                                                                   #
    # Hint: Compute e^{sim / tau} and store into exponential, which should have  #
    # shape 2N x 2N.                                                             #
    # Hint: Compute the denominator values for all augmented samples. This       #
    # should be a 2N x 1 vector.                                                 #
    # Step 2: Compute similarity between positive pairs.                         #
    # You can do this in two ways:                                               #
    # Option 1: Extract the corresponding indices from sim_matrix.               #
    # Option 2: Use sim_positive_pairs().                                        #
    # Step 3: Compute the numerator value for all augmented samples.             #
    # Step 4: Now that you have the numerator and denominator for all augmented  #
    # samples, compute the total loss.                                           #
    ##############################################################################
    ...
    # Step 3:
    numerator = torch.cat([numerator_left, numerator_right], dim=0)
    numerator = torch.exp(numerator/tau)
    
    # Step 4:
    loss_p = -torch.log(numerator/denominator)
    loss = loss_p.sum()/(2*N)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
